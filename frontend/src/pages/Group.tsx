...
import { fetchProductsByCategory, ProductRecommendation } from '../services/symptomService'
import { useCart } from '../contexts/CartContext'
...
export default function Group() {
  ...
  const dosages = useMemo(() => ['all', ...Array.from(new Set(products.map(p => p.dosage)))], [products])
  ...
  }, [products, otcFilter, brandFilter, dosageFilter])
        ...
        <Grid container spacing={2}>
          {displayed.map(p => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={p.id}>
              <Card>
                {p.image_url
                  ? <CardMedia component="img" height="140" image={p.image_url} alt={p.name} />
                  : <Skeleton variant="rectangular" height={140} />}
                <CardContent>
                  <Typography variant="h6">{p.name}</Typography>
                  <Typography variant="body2" color="textSecondary">{p.brand}</Typography>
                  <Typography variant="subtitle1">
                    {p.price_eur != null ? `${p.price_eur.toFixed(2)} €` : '— €'}
                  </Typography>
                  <Typography variant="caption" color={p.in_stock ? 'success.main' : 'error.main'}>
                    {p.in_stock ? 'In stock' : 'Out of stock'}
                  </Typography>
                </CardContent>
                <CardActions sx={{ justifyContent: 'space-between', px: 2, pb: 2 }}>
                  <Button size="small" onClick={() => add(p)} disabled={!p.in_stock}>
                    {p.in_stock ? 'Add to cart' : 'Notify me'}
                  </Button>
                </CardActions>
              </Card>
            </Grid>
